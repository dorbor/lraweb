...
app.get('/admin/allDivisions', userAuthenticated, (req, res) => {
  Division.find({ agency: 'LRA' }).then((div) => {
    Comment.find({ agency: 'LRA' }).then((comments) => {
      const complains = [];
      const applauds = [];
      comments.forEach((com) => {
...
app.get('/admin/editDivision/:id', userAuthenticated, (req, res) => {
  const id = req.params.id;
  Comment.find({ agency: 'LRA' }).then((comments) => {
    Division.findOne({ agency: 'LRA', _id: id }).then((div) => {
      const complains = [];
      const applauds = [];
      comments.forEach((com) => {
        if (com.type == 'Complain') {
...
app.get('/admin/editOfficer/:id', userAuthenticated, (req, res) => {
  ...
  Officer.findOne({ _id: id, agency: 'LRA' }).then((foundOff) => {
    Comment.find({ agency: 'LRA' }).then((comments) => {
      Position.find({ agency: 'LRA' }).then((pos) => {
        Division.find({ agency: 'LRA' }).then((div) => {
          const complains = [];
...
app.post('/admin/editOfficer/:id', userAuthenticated, (req, res) => {
  Officer.findOne({ _id: req.params.id, agency: 'LRA' }).then((foundOff) => {
    Comment.find({ agency: 'LRA' }).then((comments) => {
...
app.get('/admin/deleteOfficer/:id', userAuthenticated, (req, res) => {
  const id = req.params.id;
  Officer.findByIdAndRemove({ _id: id, agency: 'LRA' }).then((foundUser) => {
    res.redirect('/admin/allOfficers');
  });
});
...
app.get('/admin/details/:id', userAuthenticated, (req, res) => {
  ...
  Category.find({}).then((cat) => {
    Comment.find({ agency: 'LRA' }).then((comments) => {
      Comment.findOne({ _id: id, agency: 'LRA' }).then((foundCom) => {
        const complains = [];
        const applauds = [];
        comments.forEach((com) => {
          if (com.type == 'Complain') {
...
app.get('/admin/allUsers', userAuthenticated, (req, res) => {
  User.find({ agency: 'LRA' }).then((user) => {
    Officer.find({ agency: 'LRA' }).then((off) => {
      Comment.find({ agency: 'LRA' }).then((comments) => {
        const complains = [];
        const applauds = [];
...
app.get('/admin/user/:id', userAuthenticated, (req, res) => {
  const id = req.params.id;
  User.findByIdAndRemove({ _id: id, agency: 'LRA' }).then((foundUser) => {
    res.redirect('/admin/allUsers');
  });
});
...
app.route('/api/LRA/officers').get((req, res) => {
  Officer.find({ agency: 'LRA' }, (err, foundOff) => {
    if (!err) {
      res.send(foundOff);
    } else {
      res.send(err);
